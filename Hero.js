...
function Hero(program, x, y, z, degrees, bounding_cir_rad)  {
    ...
    this.heroVertices = dogMesh.vertices[0].values;

    this.heroNormals = dogMesh.vertices[1].values;

    this.heroIndices = dogMesh.connectivity[0].indices;
...
Hero.prototype.show = function() {
	...
	modelViewMatrix = mult(modelViewMatrix, rotateZ(-3)); //To better keep player-model in line with camera
	modelViewMatrix = mult(modelViewMatrix, rotateZ(-this.degrees));
